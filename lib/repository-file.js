...
module.exports = class RepositoryFile {
  ...
  // the git-host worker so the renderer thread never blocks on repository I/O.
  async load() {
    if (!this.filePath) return this;

    this.repo = await lumine.repositories.resolveForPath(this.filePath);
    ...
    if (this.shortHead) {
      configKeys.push(`branch.${this.shortHead}.remote`, `branch.${this.shortHead}.merge`);
    }
    Object.assign(this.configCache, await this.repo.getConfigValuesAsync(configKeys));

    // Remote URLs are part of the refs snapshot, including remotes without a
    // fetch refspec, so no second config worker request is needed here.
    ...
    this.configCache[remoteUrlKey] = snapshotRemote?.fetchUrl ?? null;
  ...
  // `git config lumine.open-repository.provider` wins (useful for self-hosted
  ...
  detectProvider(webURL) {
    const configured = this.getConfigValue("lumine.open-repository.provider")?.toLowerCase();
  ...
  // stay synchronous by reading this cache instead of starting another read.
  getConfigValue(key) {
  ...
  blame(lineRange) {
    if (this.validateRepo()) {
      if (this.type === "repo") {
        ...
        lumine.notifications.addWarning(`Blames do not exist for ${this.type}s`);
      }
    }
  }
  ...
  copyURL(lineRange) {
    if (this.validateRepo()) {
      lumine.clipboard.write(this.shaURL() + this.getLineRangeSuffix(lineRange));
    }
  }
  ...
  openBranchCompare() {
    if (this.validateRepo()) {
      if (this.type === "repo") {
        ...
        lumine.notifications.addWarning(`Branches do not exist for ${this.type}s`);
      }
    }
  }
  ...
  openIssues() {
    if (this.validateRepo()) {
      if (this.type === "repo") {
        ...
        lumine.notifications.addWarning(`Issues do not exist for ${this.type}s`);
      }
    }
  }
  ...
  openPullRequests() {
    if (this.validateRepo()) {
      if (this.type === "repo") {
        ...
        lumine.notifications.addWarning(`Pull requests do not exist for ${this.type}s`);
      }
    }
  }
  ...
  getLineRangeSuffix(lineRange) {
    ...
    if (!lumine.config.get("open-repository.includeLineNumbersInUrls")) {
      return "";
    }
  ...
  validateRepo() {
    if (!this.repo) {
      lumine.notifications.addWarning(`No repository found for path: ${this.filePath}.`);
      return false;
    } else if (!this.gitURL()) {
      lumine.notifications.addWarning(`No URL defined for remote: ${this.remoteName()}`);
      return false;
    }
    return true;
  }
  ...
  openURLInBrowser(url) {
    void lumine.shell.openExternal(url).catch((error) => {
      lumine.notifications.addWarning("Unable to open the repository URL.", {
  ...
  // Internal
  remoteName() {
    const gitConfigRemote = this.getConfigValue("lumine.open-repository.remote");
  ...
  // Internal
  remoteBranchName() {
    const gitConfigBranch = this.getConfigValue("lumine.open-repository.branch");
